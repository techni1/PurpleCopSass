...
const SearchTable = ({ tableData, index = 0 }: any) => {
  ...
  const handleDeleteClick = (employee: any) => {
    ...
    setModalMessage(`Sass Package ${employee.name} will be deleted !!`);
    setEmployeeToDelete(employee);
    setShowModal(true);
  };


  const handleSasspackage = (sasspackage: any) => {

    if (sasspackage) {
      setShow(true);
      setSPackage(sasspackage);
    }
  }
  ...
  const deleteEmployee = () => {
    if (employeeToDelete) {
      router.delete(route("sasspackage.destroy", employeeToDelete));
      setShowModal(false);
    }
  };
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        cell: (info: any) => (
          <Link
            href={route("sasspackage.show", info.row.original.id)}
      ...
      {
        header: "Grace Period",
        accessorKey: "grace_period_day",
        enableColumnFilter: false,
      },
      {
        header: "No Of Users",
        accessorKey: "no_of_user",
        enableColumnFilter: false,
      },
          ...
          <Dropdown>
            ...
            <Dropdown.Menu className="dropdown-menu-end">
              <Dropdown.Item
                ...
                onClick={() => handleSasspackage(info.row.original)}
              >
