...
const SearchTable = ({ tableData, index = 0 }: any) => {
  ...
  const handleDeleteClick = (employee: any) => {
    ...
    setModalMessage(`Employee ${employee.name} will be deleted !!`);
  ...
  const deleteEmployee = () => {
    if (employeeToDelete) {
      router.delete(route("people.destroy", employeeToDelete));
      setShowModal(false);
    }
  };
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        accessorKey: "installed_antivirus",
        enableColumnFilter: false,
      },
      {
        header: "Actions",
