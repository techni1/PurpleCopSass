...
const SearchTable = ({ routeTo, tableData, index = 0 }: any) => {
  ...
  const deleteClient = () => {
    router.delete(route("riskcategory.destroy", categoryToDelete));
    setShowModal(false);
    // toprightnotify();
  };
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        cell: (info: any) => (
          <Link
            href={route("riskcategory.show", info.row.original.id)}
          ...
          <Dropdown>
            ...
            <Dropdown.Menu className="dropdown-menu-end">
              <Dropdown.Item className="dropdown-item edit-item-btn">
                <Link href={route("riskcategory.edit", info.row.original.id)}>
