...
const SearchTable = ({ routeTo, tableData, index = 0 }: any) => {
  ...
  const [functionalGroupToDelete, setFunctionalGroupToDelete] = useState("");

  const handleDeleteClick = (functionalGroup: any) => {
    setModalTitle("Are You Sure");
    setModalMessage(
      `FunctionalGroup ${functionalGroup.name} will be deleted !!`
    );
    setFunctionalGroupToDelete(functionalGroup);
    setShowModal(true);
  };
  ...
  const deleteClient = () => {
    router.delete(route("functionalgroup.destroy", functionalGroupToDelete));
    setShowModal(false);
  };
