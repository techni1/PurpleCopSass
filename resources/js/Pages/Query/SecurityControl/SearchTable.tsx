...
const SearchTable = ({ routeTo, tableData, index = 0 }: any) => {
  ...
  const handleDeleteClick = (category: any) => {
    ...
    setModalMessage(`Query "Category ${category.name}" will be deleted !!`);
    setCategoryToDelete(category);
    setShowModal(true);
  };
  const deleteClient = () => {
    router.delete(route("category.destroy", categoryToDelete));
    setShowModal(false);
    // toprightnotify();
  };
  ...
  const columns = useMemo(
    () => [
...

      {
        ...
        cell: (info: any) => (
          <Link
            href={route("securitycontrol.show", info.row.original.controls)}
