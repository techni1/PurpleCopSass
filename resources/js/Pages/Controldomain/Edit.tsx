...
export default function Create({ controldomain }: any) {
  const { data, setData, post, processing, errors, reset } = useForm({
    id: controldomain.id,
    name: controldomain.name || "",
    description: controldomain.description || "",
    _method: "PUT",
  });

  const onSubmit = (e: any) => {
    e.preventDefault();
    post(route("controldomain.update", controldomain.id), {
      preserveScroll: true,
      onSuccess: () => reset(),
    });
  };
                ...
                <Card.Body>
                  <form onSubmit={onSubmit}>
                    ...
                    <Row className="justify-content-md-center">
                      <Col xl={2} md={6}>
                        <Link
                          href={route("controldomain.index")}
