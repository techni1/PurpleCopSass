...
import AuditDetail from "../../AuditCenter/AuditDetail";
...
export default function PolicyList({
  ...
  const [currentUserPolicy, setCurrentUserPolicy] = useState([]);
  ...
  const handleDeleteClick = (policy: any) => {
    ...
    setModalMessage(policy.policy.name);
  ...
  const deleteClient = () => {
    router.delete(route("organizationpolicy.destroy", policyToDelete));
    setShowModal(false);
  };

  useEffect(() => {
    const policy = listData.filter(
      (data: any) =>
        data.assignee?.id === user.id || data.approver?.id === user.id
    );
    setCurrentUserPolicy(policy);
  }, [listData, user.id]);
        ...
        cell: (info: any) => (
          <div>
            {info.row.original.scope == "in" ? (
              <Link
                href={route("organizationpolicy.show", info.row.original.id)}
        ...
        cell: (info: any) => (
          <Dropdown>
            ...
            <Dropdown.Menu className="dropdown-menu-end">
              ...
              {info.row.original.policy.source != "InstaGRC" && (
                <Dropdown.Item className="dropdown-item edit-item-btn">
                  <Link
                    href={route("policy.edit", info.row.original.policy.id)}
                  >
              ...
              {info.row.original.approver_status != "approved" && (
                <ScopeChange orgPolicy={info.row.original} />
              )}
            </Dropdown.Menu>
          </Dropdown>
        ),
      ...
      <Modal show={showModal} onHide={handleCloseModal} centered>
        ...
        <Modal.Body className="text-center p-5">
          ...
          <div className="mt-4">
            ...
            <h5> {modalMessage}</h5>
            <p className="text-danger mb-4">Delete Policy</p>
