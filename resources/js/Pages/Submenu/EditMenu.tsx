...
const EditMenu = ({ show, setShow, menu, submenuId }: any) => {
    ...
    useEffect(() => {
        if (submenuId) {
            ...
            axios.get(route("submenu.edit", submenuId)).then((response) => {
                const menuData = response.data;
                setData({
                    name: menuData.name || "",
                    url: menuData.url || "",
                    menuid: menuData.menuid !== undefined ? menuData.menuid : "",
                    status: menuData.submenu_status !== undefined ? menuData.submenu_status : "",
                });
            }).catch(error => {
    ...
    const onSubmit = (e: any) => {
        e.preventDefault();
        put(route("submenu.update", submenuId), {
            onSuccess: () => {
                reset();
                            ...
                            <div className="row">
                                ...
                                <div className="col-md-12 pt-3">
                                    ...
                                    <select
                                        ...
                                        onChange={(e: any) => setData("menuid", e.target.value)}
                                        name="menuid"
                                        value={data.menuid}
                                    >
