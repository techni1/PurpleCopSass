...
const SearchTable = ({ routeTo, tableData, assignees }: any) => {
  ...
  const [userPolicyToDelete, setUserPolicyToDelete] =
    useState<UserPolicyData | null>(null);
  const [showToChangeModal, setShowToChangeModal] = useState(false);
  const [userPolicyToUpdate, setUserPolicyToUpdate] =
    useState<UserPolicyData | null>(null);

  const handleDeleteClick = (userpolicy: UserPolicyData) => {
    setModalTitle("Are You Sure");
    setModalMessage(
      `User Policy: ${userpolicy.policyId.name} of Entity: ${userpolicy.entityId.name} will be deleted !!`
    );
    setUserPolicyToDelete(userpolicy);
    setShowDeleteModal(true);
  };

  const handleStatusChange = (userpolicy: UserPolicyData) => {
    setToUpdate("STATUS");
    setUserPolicyToUpdate(userpolicy);
    setShowToChangeModal(true);
  };
  const handleAssigneeChange = (userpolicy: UserPolicyData) => {
    setToUpdate("ASSIGNEE");
    setUserPolicyToUpdate(userpolicy);
    setShowToChangeModal(true);
  };
  const handleRecurrenceChange = (userpolicy: UserPolicyData) => {
    setToUpdate("RECURRENCE");
    setUserPolicyToUpdate(userpolicy);
    setShowToChangeModal(true);
  };
  const handleReviewDateChange = (userpolicy: UserPolicyData) => {
    setToUpdate("REVIEW");
    setUserPolicyToUpdate(userpolicy);
    setShowToChangeModal(true);
  };
  ...
  const deleteClient = () => {
    if (userPolicyToDelete) {
      router.delete(route("userpolicy.destroy", userPolicyToDelete.id));
      setShowDeleteModal(false);
    }
  };
  ...
  useEffect(() => {
    if (userPolicyToUpdate) {
      setData({
        status: userPolicyToUpdate.status,
        assignee_id: userPolicyToUpdate.assigneeId
          ? userPolicyToUpdate.assigneeId.id
          : "",
        recurrence: userPolicyToUpdate.recurrence,
        review_date: userPolicyToUpdate.review_date,
        approver_id: userPolicyToUpdate.approverId
          ? userPolicyToUpdate.approverId.id
          : "",
      });
      if (userPolicyToUpdate.review_date) {
        setDate(new Date(userPolicyToUpdate.review_date));
      }
    }
  }, [userPolicyToUpdate]);
  ...
  const onStatusSubmit = (e: any) => {
    e.preventDefault();
    if (userPolicyToUpdate) {
      patch(route("userpolicy.update", userPolicyToUpdate.id), {
        preserveScroll: true,
        onSuccess: () => {
          reset();
          setShowToChangeModal(false);
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        cell: (info: any) => (
          <Link
            href={route("entity.show", info.row.original.entityId)}
