...
const SearchTable = ({ routeTo, tableData }: any) => {
  ...
  const handleDeleteClick = (role: any) => {
    ...
    setModalMessage(`Role ${role.name} will be deleted !!`);
  ...
  const deleteClient = () => {
    router.delete(route("role.destroy", roleToDelete));
    setShowModal(false);
  };
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        cell: (cell) => (
          <>
            {cell.getValue().map((permission: any) => (
              <div>
                <span
                  key={permission.id}
          ...
          <Dropdown>
            ...
            <Dropdown.Menu className="dropdown-menu-end">
              <Dropdown.Item
                ...
                onClick={() =>
                  (window.location.href = route("role.edit", info.row.original))
                }
              >
