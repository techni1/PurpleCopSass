...
class ReortPoilcyExport implements FromQuery, WithHeadings, WithTitle
{
    use Exportable;
    ...
    public function query()
    {
        $data = DB::table('frameworks')
            ...
            ->leftJoin('organization_policies', 'organization_policies.evidence_id', '=', 'control_code_policies.evidence_id')
            ->select(
            ...
            ->where('organization_policies.policy_id', '!=', '')
        ...
        if ($this->status == 'pending') {
            $data->where(function ($query) {
                $query->where('organization_policies.assignee_status', '=', 'pending')
                    ->orWhere('organization_policies.approver_status', '=', 'pending');
            });
